...
class HomeVisitWidget extends SalesdocketConsumerWidget {
  const HomeVisitWidget({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final selectedFollowupStatus = ref.watch(
      leadFilterRequestProvider.select((req) => req?.followupStatus),
    );
    final filterItems = LeadFiltersUtils.homeVisitFilters;

    return CheckListWidget(
      items: filterItems,
      isMultiSelect: false,
      selectedItems:
          selectedFollowupStatus == null ? [] : [selectedFollowupStatus],
      onChanged: (selected) {
        ref
            ...
            .update(
              (toUpdate) =>
                  toUpdate = toUpdate?.copyWith(
                    followupStatus: selected.firstOrNull,
                  ),
            );
      },
    );
