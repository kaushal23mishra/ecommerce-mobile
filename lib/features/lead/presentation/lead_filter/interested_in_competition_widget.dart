...
class InterestedInCompetitionWidget extends SalesdocketConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final selectedDmsId = ref.watch(
      leadFilterRequestProvider.select((req) => req?.interestedInCompetition),
    );
    final filterItems = LeadFiltersUtils.interestedInCompetitionFilters;
            ...
            .update(
              (toUpdate) =>
                  toUpdate = toUpdate?.copyWith(
                    interestedInCompetition: selected.firstOrNull,
                  ),
            );
      },
    );
