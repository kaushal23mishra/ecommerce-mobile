...
class LeadTypeWidget extends SalesdocketConsumerWidget {
  const LeadTypeWidget({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final selectedLeadSType = ref.watch(
      leadFilterRequestProvider.select((req) => req?.leadState),
    );
    final filterItems = LeadFiltersUtils.leadTypeFilters;

    return CheckListWidget<String>(
      items: filterItems,
      selectedItems: selectedLeadSType?.split(",") ?? [],
      onChanged: (selected) {
        ref
            ...
            .update(
              (toUpdate) =>
                  toUpdate = toUpdate?.copyWith(
                    leadState: selected.isEmpty ? null : selected.join(","),
                  ),
            );
      },
    );
