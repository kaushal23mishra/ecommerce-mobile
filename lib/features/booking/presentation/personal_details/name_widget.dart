...
class NameWidget extends SalesdocketConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final selectedSalutation = ref.watch(
      selectedBookingProvider.select((state) => state?.salutation),
    );
    final fullName =
        ref
            .watch(selectedBookingProvider.select((state) => state))
            ?.fullNameWOSalutation ?? ref.read(bookingLeadRequestProvider)?.bookingFullNameWOSalutation;
    final error = ref
