...
class _ActionState extends SalesdocketConsumerState<ActionWidget>
  ...
  void _validateAndSubmitRequest() {
    if (_isValidForm()) {
      final lead = ref.read(inactiveBookingLeadProvider);
      final request = ref.read(inactiveBookingRequestProvider);
  ...
  bool _isValidForm() {
    final inactiveBookingRequest = ref.read(inactiveBookingRequestProvider);
    final errors =
        inactiveBookingRequest?.inactiveBookingValidationErrors() ?? [];

    if (errors.isNotEmpty) {
      showSnackBar(errors.firstErrorMessage);
      ref.read(inactiveBookingFormErrorsProvider.notifier).addAll(errors);
    }
  ...
  void onLeadStatusChanged(String? leadStatus) async {
    ...
    if (lead?.hasBookingFollowup == true) {
      ...
      showSnackBar(LocaleKeys.leadInactivated.tr(), type: SnackBarType.success);
