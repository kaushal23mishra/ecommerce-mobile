...
internal class FlutterPhoneDirectCallerHandler :
    ...
    fun setActivityPluginBinding(activityPluginBinding: ActivityPluginBinding) {
        // Remove from old binding first to prevent registering the listener twice,
        // which causes "Reply already submitted" when onRequestPermissionsResult fires
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode != CALL_REQ_CODE) return false
        number?.let {
            val result = flutterResult
            flutterResult = null  // clear before calling to prevent double-reply crash
